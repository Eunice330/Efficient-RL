...
class MPC(Module):
    ...
    def forward(self, x_init, cost, dx):
        ...
        if self.u_init is None:
            ...
            print('self T', self.T, 'self.n_batch', n_batch, 'self.n_ctrl', self.n_ctrl)
            u = np.zeros((self.T, n_batch, self.n_ctrl)) #default init u
            #u = np.ones((self.T, n_batch, self.n_ctrl))
            #u = 1.2 * np.ones((self.T, n_batch, self.n_ctrl))
        else:
            u = self.u_init
            if u.ndimension() == 2:
        ...
        for i in range(self.lqr_iter):#use lqr to find best trajectory:forward part
            ...
            #F, f = self.linearize_dynamics(x, u, dx, diff=True)
            #print('lqr iter',i, 'F shape', np.array(F).shape)
            #print('lqr iter', i, 'f shape', np.array(f).shape)
        ...
        #C, c, _ = self.approximate_cost(x, u, cost, diff=True) # use our cost
    ...
    def solve_lqr_subproblem(self, x_init, C, c, F, f, cost, dynamics, x, u,
                             no_op_forward=False):
        #if not dynamics.isPendulum:
        C = Variable(torch.from_numpy(np.array(C)).float())
        c = Variable(torch.from_numpy(np.array(c)).float())
        F = Variable(torch.from_numpy(np.array(F)).float())
        f = Variable(torch.from_numpy(np.array(f)).float())
...
""")
            ...
            for t in range(self.T):
                batch_Ct, batch_grads, batch_hessians = our_gp_cost.get_cost(x[t], u[t])
            ...
            print('all batch costs', np.array(costs).shape) 
                ...
                #print('cost sum', cost.sum().shape, cost.sum())
                #hessian = list()
    ...
    def linearize_dynamics(self, x, u, dynamics, diff):
        ...
        x = np.array(x)
        u = np.array(u)
        #print('input linear dynamics x shape', x.shape)
        #print('input linear dynamics u shape', u.shape)
        ...
        if self.grad_method == GradMethods.ANALYTIC:   #GP MODEL
            ...
            for t in range(self.T):
                if t < self.T-1:
                    ...
                    xt = Variable(torch.Tensor(x[t]), requires_grad=True)
                    #ut = Variable(u[t], requires_grad=True)
                    ut = Variable(torch.Tensor(u[t]), requires_grad=True)
                    ...
                    if self.grad_method in [GradMethods.AUTO_DIFF, GradMethods.ANALYTIC_CHECK]: #li
                        Rt, St = [], []
                        for j in range(self.n_state):
                            Rj, Sj = torch.autograd.grad(
                                ...
                                retain_graph=True)
                            #if j==0:
                                  #print('new x[:,0]', new_x[:,j].shape, new_x[:,j])
                                  #print('new_x[:,0]sum', new_x[:,j].sum().shape, new_x[:,j].sum())
                                  #print('R0 shape', Rj.shape)
                                  #print('S0 shape', Sj.shape)
                            if not diff:
