...
class LQRStep(Function):
    ...
    def lqr_forward(self, x_init, C, c, F, f, Ks, ks):
        x = self.current_x
        u = self.current_u
        #print('u input of lqr forward', u)
        n_batch = C.size(1)

        #old_cost = util.get_cost(self.T, u, self.true_cost, self.true_dynamics, x=x)
        old_cost = util.get_cost(self.T, u, C, c, self.true_cost, self.true_dynamics, x=x)
        ...
        while (current_cost is None or \
            ...
            for t in range(self.T):
                ...
                if self.u_lower is not None:
                    ...
                    #print('new ut after line 378', new_ut)
                new_u.append(new_ut)

                new_xut = torch.cat((torch.Tensor(new_xt), torch.Tensor(new_ut)), dim=1)#
                if t < self.T-1:
                    if isinstance(self.true_dynamics, mpc.LinDx):
                        ...
                        #print('new xt', new_xt)
                        #print('new ut', new_ut)
                        _, new_xtp1 = self.true_dynamics.grad_input(new_xt, new_ut)# 

                    new_x.append(new_xtp1)
                    #print('new xtp1 shape', np.array(new_xtp1).shape)
                    dx.append(torch.Tensor(new_xtp1) - x[t+1])
            ...
            #print('new u', new_u.shape, new_u) 
            #new_x = torch.stack(new_x)
            #print('new_x.remove(new_x[0])', torch.Tensor(new_x.remove(new_x[0])).shape, torch.Tensor(new_x.remove(new_x[0])))
            #print('delete first tensor shape', torch.Tensor(new_x[1:]).shape)
            #print('newx[0] shape', new_x[0].unsqueeze(0).shape)
            new_x = torch.cat((new_x[0].unsqueeze(0), torch.Tensor(new_x[1:])),0) #change new_x to torch.Tensor
